...
def process_nifti(mask_path, vol_path, output_dir, subject_id, T2S_only, task):
    ...
    for z in range(original_volume.shape[2]):   # Iterate through slices
        ...
        Image.fromarray(mask_slice, mode="L").save(os.path.join(output_dir, 'masks', task, f'{subject_id}_slice_{z:03d}.png'))
...
def main(T2S_only):
    dir_for_mask_and_split = "/media/Datacenter_storage/PublicDatasets/cerebral_microbleeds_VALDO/cmb_csf"
    ...
    output_dir = "/media/Datacenter_storage/PublicDatasets/cerebral_microbleeds_VALDO/csf_png"
