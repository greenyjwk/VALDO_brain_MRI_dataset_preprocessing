...
def extract_slices(nifti_data, single_channel, sequence):
    
    if sequence == "Axial_3TE_T2_STAR":
        print("This is T2S sequence")
...
def save_slices_as_png(slices, output_dir, base_name, c):
...
def main(single_channel, target_sequence):
    ...
    parser.add_argument("--nifti_dir", type=str, default='/media/Datacenter_storage/PublicDatasets/MAYO_cerebral_microbleeds/mayo_allsequence')
    parser.add_argument("--output_dir", type=str, default='/media/Datacenter_storage/PublicDatasets/MAYO_cerebral_microbleeds/mayo_t1')
    ...
    for mrn in os.listdir(nifti_dir):
        ...
        for nifti_file in os.listdir(os.path.join(nifti_dir, mrn)):
            ...
            print(nifti_file)
            print()
            if nifti_file.startswith("3D_SAG_T1_MPRAGE_1MM") and target_sequence == "3D_SAG_T1_MPRAGE_1MM":
                print("This is T1: ", nifti_file)
                nifti_path = os.path.join(nifti_dir, mrn, nifti_file)
                print("nifti_path: ", nifti_path)
                nifti_img = nib.load(nifti_path)
            elif nifti_file.startswith("SAG_T2_SPACE") and target_sequence == "SAG_T2_SPACE":
                print("This is T2: ", nifti_file)
                nifti_path = os.path.join(nifti_dir, mrn, nifti_file)
                nifti_img = nib.load(nifti_path)
            elif nifti_file.startswith("Axial_3TE_T2_STAR") and target_sequence == "Axial_3TE_T2_STAR":
                print("This is T2 STAR: ", nifti_file)
                nifti_path = os.path.join(nifti_dir, mrn, nifti_file)
                nifti_img = nib.load(nifti_path)
            ...
            if not os.path.exists(nifti_path):
                ...
                continue
            slices = extract_slices(nifti_data, single_channel, target_sequence)
            base_name = nifti_file.split('.')[0]
            save_slices_as_png(slices, output_dir, base_name, single_channel)
...
if __name__ == "__main__":
    ...
    # T1 = 3D_SAG_T1_MPRAGE_1MM
    # T2 = SAG_T2_SPACE
    # T2S = Axial_3TE_T2_STAR
    target_sequence = "3D_SAG_T1_MPRAGE_1MM"
    main(single_channel, target_sequence)
