...
def yolo_to_coco(yolo_dataset_path, output_path, categories=None):
    ...
    for ext in image_extensions:
        image_paths.extend(glob.glob(os.path.join(yolo_dataset_path, 'images/test', ext)))
    ...
    for img_id, img_path in enumerate(tqdm(image_paths, desc="Converting")):
        ...
        ann_path = os.path.join(yolo_dataset_path, 'labels/test', f"{img_name_without_ext}.txt")
...
# python  /media/Datacenter_storage/Ji/VALDO_brain_MRI_dataset_preprocessing/src/preprocess/yolo2coco_mayo.py   --mode yolo2coco --input /media/Datacenter_storage/Ji/brain_mri_valdo_mayo/mayo_yolo_all_sequence --output /media/Datacenter_storage/Ji/brain_mri_valdo_mayo/mayo_yolo_all_sequence_coco/annotations.json --categories cmb