...
def process_all_subjects(original_data_dir, preprocessed_img_dir, output_dir):
    ...
    for train_mask_path in train:
        ...
        vol_path = os.path.join(preprocessed_img_dir, train_subject_id, f'{train_subject_id}_space-T2S_desc-masked_T2S.nii.gz')
        train_mask_path =  os.path.join(original_data_dir, train_subject_id, train_mask_path)
...
def get_train_val(root_dir):
    ...
    for subdir, _, files in os.walk(root_dir):
        for file in files:
            if file.endswith('T2S_CMB.nii.gz'):
...
def main():
    ...
    # preprocessed_img_dir = "/mnt/storage/cmb_segmentation_dataset_preprocessed/bias_field_correction"
    preprocessed_img_dir = "/mnt/storage/ji/VALDO_preprocessing/bias-field-correction"
    output_dir = "/mnt/storage/ji/new_preprocessed_valdo"
